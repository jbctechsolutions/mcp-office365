...
export function buildHttpApp(options: HttpServerOptions): Express {
  ...
  // CORS: a browser-based MCP client (claude.ai) fetches the discovery metadata
  // and posts to /mcp cross-origin, so the well-known + /mcp responses must be
  // readable by the browser. Authorization is a header (not a cookie), so a
  // wildcard origin is safe and no credentials mode is needed. WWW-Authenticate
  // must be exposed so the 401 challenge (with the PRM URL) is readable.
  ...
  app.post('/mcp', ...mcpChain, async (req: Request, res: Response): Promise<void> => {
    ...
    // narrowed access take effect on the next call, incl. confirm_*). When auth
    // is on, always resolve — an (unreachable) missing identity uses a sentinel
    // oid, which yields the pinned default surface, NEVER the full surface.
    const surface =
      options.auth != null
        ? options.auth.entitlements.resolve(req.remoteIdentity?.oid ?? ' unidentified', 'graph')
        : undefined;
    const perRequestOptions: ServerOptions =
      options.auth != null
        ? {
            ...
            // Audit (U8): record write/destructive calls under this user's
            // identity into the shared durable store. Only with a real identity —
            // the (unreachable) missing-identity case is not attributed.
            ...(req.remoteIdentity != null
              ? {
                  audit: {
                    store: options.stateStore,
                    oid: req.remoteIdentity.oid,
                    tid: req.remoteIdentity.tid,
                  },
                }
              : {}),
